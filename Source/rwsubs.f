...
c     type of exchange and correlation
      read (input,*) z,xion,exfact
c davegp
c I'm changing this format statement since it forces z to be integer
  210 format (f5.0,2f10.5)
c  210 format (f7.2,2f10.5)
      write(iout,*) 'z, xion, exfact', z, xion, exfact
c
c
...
c     perform a self-consistency check
  ...
  245   format(6h www= ,f6.2,7h xion= ,f6.2,6h   z= ,f6.2,
     +    11h   ncores= ,i3,11h   nvales= ,i3,11h   ncspvs= ,i3/
...
c     print out of all electron control parameters
  ...
  260 format (' z =',f6.2,4x,'xion =',f5.2,4x,
...
c
        ...
        elseif ( ifqopt .lt. 0) then
          write (iout,*) ' q-functions will not be pseudized'
        else
          write(iout,*) '***error in subroutine readin'
...
c
 ...
 1020 format (4x,'|  z =',f7.2,2x,'zv =',f7.2,2x,'exfact =',f10.5,
     +   13x,'|')
...
c davegp
c print out all states
        write(iout,*) ' davegp - begin printing all-electron states'
        write(iout,*) ncspvs
        do iorb = 1,ncspvs
           lcur = nnlz(iorb) / 10 - 10 * ( nnlz(iorb) / 100 )
           write(iout,*) mesh, lcur, nnlz(iorb)
           do ir=1,mesh
              write(iout,'(2e20.12)') r(ir),snl(ir,iorb)
           enddo
        enddo
        write(iout,*) ' davegp - end printing all-electron states'
c davegp
c
...
c
      if ( ipass .eq. 2 ) then
c
c       write (iout,400) (nnlz(mp),mp=ncores+1,ncspvs)
c 400   format (/,'   all electron and pseudo energies',//,12x,
c    +    4(11x,i3,11x))
c       write (iout,410) ('   full     pseudo  ',i=1,nvales)
c 410   format (/15x,4(2x,a20,3x)/)
c       write (iout,420) 'energy',(eeprt(i),i=1,2*nvales)
c 420   format (4x,a6,2x,4(2x,f10.7,1x,f10.7,2x))
c
        write (iout,'(/1x,a )')
     +    '------------------------------------------------------'
        write (iout,'( 1x,a )')
     +    'comparison of all-electron and pseudo eigenvalues (Ry)'
        write (iout,'(/   a/)')
     +    '   nlm    all-elec      pseudo        diff'
        do i=1,nvales
          ...
          eeae =eeprt(2*i-1)
          eeval=eeprt(2*i)
          write (iout,'(i6,3f12.6)') nnlz(mp),eeae,eeval,eeae-eeval
        end do
        write (iout,'(/1x,a )')
     +    '------------------------------------------------------'
c
