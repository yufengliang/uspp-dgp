...
c davegp
c          write(iout,*) ibeta
c          write(iout,'(4a25)') 'r', 'psi', 'phi', 'chi'
c          write(iout,*) kkbeta, lcur
c          do i=1,kkbeta
c            write(iout,'(4e20.12)') r(i), psi(i,ibeta), phi(i,ibeta),
c     +        chi(i,ibeta)
c          enddo
c davegp
...
c davegp
          write(iout,*) ibeta
          write(iout,'(4a25)') 'r', 'psi', 'phi', 'chi'
          write(iout,*) kkbeta, lcur
          do i=1,kkbeta
            write(iout,'(4e20.12)') r(i), psi(i,ibeta), phi(i,ibeta),
     +        chi(i,ibeta)
          enddo
c davegp
c
...
c
c       obtain eigenvalues of q matrix
        if (nn.eq.1) then
          qeig(ns)  =qqq(ns,ns)
        else if (nn.eq.2) then
          qbar=(qqq(ns+1,ns+1)+qqq(ns,ns))/2.
          qdif=(qqq(ns+1,ns+1)-qqq(ns,ns))/2.
          qoff= qqq(ns+1,ns)
          qdel=sqrt(qdif**2+qoff**2)
          qeig(ns)  =qbar-qdel
          qeig(ns+1)=qbar+qdel
        else if (nn.gt.2) then
          do is=ns,ns+nn-1
            qeig(is)=0.
          end do
          write (iout,*) ' *** warning: not implemented ***'
        endif
c
  470 continue
c
      ...
      call prmat(qqq,nbeta,'q',idim3)
c
      write (iout,'(/6x,a//(7f14.5))') 'eigenvalues of q matrix',
     +   (qeig(ibeta),ibeta=1,nbeta)
c
      call prmat(ddd,nbeta,'d',idim3)
c
c---------------------------------------------------------------------------
...
c---------------------------------------------------------------------------
c
c     print warnings if eigenvalues of q matrix are unreasonable
      qqmin=qeig(1)
      qqmax=qeig(1)
      if (nbeta.gt.1) then
        do ibeta=2,nbeta
          qqmin=min(qqmin,qeig(ibeta))
          qqmax=max(qqmax,qeig(ibeta))
        end do
      endif
! davegp - I switched this qqmin .lt. -1.0 to -2.0
      if (qqmin.lt.-2.0) then
        write (iout,*) ' *** error: unreasonable q eigenvalues'
        call exit(1)
      endif
      if (qqmax.gt.50.0) then
        write (iout,*) ' *** error: max q eigenvalue too large'
        call exit(1)
      else if (qqmax.gt.20.0) then
        write (iout,'(/a)')
     +     ' ** severe warning: max q eigval very large **'
      else if (qqmax.gt.5.0) then
        write (iout,'(/a)')
     +     ' ** mild warning: max q eigval somewhat large **'
      endif
c
      write (stderr,*) 'transformation of q and d complete'
c
c davegp
      call prmat(ttt,nbeta,'i',idim3)
c davegp
      write(iout,*) ' transformed atomic waves - davegp'
      write(iout,'(4a25)') 'r', 'transformed phi then psi'
      write(iout,*) nbeta
      do ibeta=1,nbeta
c phi
        dum1=0.d0
        write(iout,*) kkbeta, lll(ibeta)
        do i=1,kkbeta
          do jbeta=1,nbeta
            dum1(i)=dum1(i)+ttt(ibeta,jbeta)*phi(i,jbeta)
          enddo
        enddo
        do i=1,kkbeta
          write(iout,'(2e20.12)') r(i), dum1(i)
        enddo
c psi
        dum1=0.d0
        write(iout,*) kkbeta, lll(ibeta)
        do i=1,kkbeta
          do jbeta=1,nbeta
            dum1(i)=dum1(i)+ttt(ibeta,jbeta)*psi(i,jbeta)
          enddo
        enddo
        do i=1,kkbeta
          write(iout,'(2e20.12)') r(i), dum1(i)
...
c
c
          if ( ifprt .ge. 2) write (iout,'(6x,a,f14.8,e12.4)')
...
c
        if ( ifprt .eq. 1)
     +       write (iout,'(6x,a,f14.8,e12.4,a,i4,a)')
     +       'ecur,decur',ecur,decur,' in',iter,' iterations'
c
c       update the energy
        ee(iorb) = ecur
c
