...
c
  ...
  210     continue
          ...
          if (ifprt.gt.1) write (iout,'(3x,a,i2,f12.8)')
     +      'intermed rsvale for lp=',lp,asum
          if (ifprt.gt.1) write (iout,'(5x,a,2f12.8)')
...
c
  ...
  240     continue
          call radin(mesh,dum,rab,asum,idim1)
          if (ifprt.gt.1) write (iout,'(5x,a,i2,f12.8)')
...
c
        call radin(mesh,dum,rab,asum,idim1)
c
        if (ifprt.gt.0) write (iout,'(3x,a,f12.8,a,f12.8)')
...
c
        if ( abs( asum - wwsum ) .gt. 8.0d-05 ) then
          ...
          call exit(1)
c         (*) see note below for explanation of why this may happen
        endif
c
...
c
c     (*) Note:
c     If the error 'asum and wwsum are not consistent' occurs, it
c     is because (qqq_ij) is not exactly equal to (int qfunc_ij(r)
c     dr).  The cause of this, in turn, can be traced to the
c     pseudization of the qfunc that occurs in psqf1 or psqf2;
c     those routines print out a comparison of `conae' and `conps'
c     which are essentially the target and actual value of the
c     integrated-charge constraint, which should in principle be
c     exactly equal, but numerically they never are. The
c     discrepancy tends to be unacceptably large when the
c     pseudization of the qfunc is "unnatural", e.g., unreasonalbe
c     constraints have been imposed.  Set ifprt.ge.2 and check
c     the plots of the qfunc pseudization; probably they will show
c     signs of abnormality that can be fixed by adjusting rinner or
c     playing with extra constraints, after which this problem will
c     hopefully go away.    -- dv 7/02
c
      return
      end
c
...
c
      implicit double precision(a-h,o-z)
c
      parameter ( idim10 = 101 , idim11 = 10 )
c
c.....logarithmic radial mesh information
      dimension r(idim1),rab(idim1),sqr(idim1)
c.....potentials for the all electron calculation
